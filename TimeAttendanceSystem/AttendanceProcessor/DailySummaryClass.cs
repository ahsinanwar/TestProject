...
namespace TimeAttendanceSystem.AttendanceProcessor
{
    public class DailySummaryClass
    {
        ...
        public DailySummaryClass(DateTime dateStart, DateTime dateEnd)
        {
            ...
            wings = db.Departments.ToList();
            List<Holiday> holidays = new List<Holiday>();
            holidays = db.Holidays.ToList();
            while (dateStart <= dateEnd)
            {
                if (dateStart.Date.DayOfWeek == DayOfWeek.Saturday || dateStart.Date.DayOfWeek == DayOfWeek.Sunday)
                {
                ...
                else
                {
                    ...
                    if (holidays.Where(aa => aa.HolDateFrom >= dateStart && aa.HolDateTo <=dateStart).Count() > 0)
                    {
                    ...
                    else
                    {
                        ...
                        foreach (var location in locs)
                            CalculateSummary(dateStart, "L", location.LocID);
                        foreach (var section in secs)
                            CalculateSummary(dateStart, "E", section.SectionID);
                        foreach (var wing in wings)
                            CalculateSummary(dateStart, "D", wing.DeptID);
                        foreach (var shift in shifts)
                            CalculateSummary(dateStart, "S", shift.ShiftID);
        ...
        public static void CalculateSummary(DateTime dateStart, String criteria, int criteriaValue)
        {
            ...
            if (attList.Count > 0)
            {
                ...
                { }
                if (ProcssedDS == false)
                    context.DailySummaries.Add(dailysumm);
                context.SaveChanges();
