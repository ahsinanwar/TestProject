...
namespace TASDownloadService.Helper
{
    class Downloader
    {
        public List<string> GetFpIDFromDevice(Reader reader)
        {  
            
         List<ReadersCommLibrary.Poll> records = new List<ReadersCommLibrary.Poll>();
         List<string> listOfEmps = new List<string>();
                ...
                if (IsConnectedToInternet(reader.IpAdd))
                {
                    ...
                    if (readerHelper.Connect(reader.IpAdd, reader.IpPort))
                    {
                        try
                        {
                            ...
                            foreach (string fpid in readerHelper.GetUsersFromDevice())
                            {
                                listOfEmps.Add(fpid);
                            
                            
                            }
                          //  vMReader.listOfShiftEmps = listOfEmps;
                            return listOfEmps;
                        }
                        catch (Exception e)
                        {
                    ...
                    if (IsConnectedToInternet(reader.IpAdd))
                    {
                        ...
                        if (readerHelper.Connect(reader.IpAdd, reader.IpPort))
                        {
                            try
                            {
                                ...
                                records = readerHelper.DownloadData();
        ...
        private void SaveServiceLog(short _RdrID,string _Description,byte _ErrorCode)
        {
            try
            {
                using (var context = new TAS2013Entities())
                {
                    ...
                    context.ServiceLogs.Add(_ServiceLog);
                    context.SaveChanges();
                }
            }
            catch (Exception ex)
        ...
        private void SaveRdrEventLog(short _RdrID, string _Description, byte _ErrorCode)
        {
            using (var context = new TAS2013Entities())
            {
                ...
                context.RdrEventLogs.Add(_rdrEventLog);
                context.SaveChanges();
            }
        }
        ...
        private void SaveAttDataToPollData(List<ReadersCommLibrary.Poll> records, Int16 _RdrID)
        {
            using (var context = new TAS2013Entities())
            {
                ...
                foreach (var entry in records)
                {
                    try
                    {
                        var emp = emps.Where(aa => aa.FpID == entry.FPID);
